...
def callback_bluetooth(message):
    ...
    if flag_manul == 1:
        print "Received from bluetooth: header=%d,direction=%d,speed=%d,gear==%d,manual=%d,beep=%d,crc=%d" % (ord(message.data[0]), ord(message.data[1]), ord(message.data[2]), ord(message.data[3]), ord(message.data[4]), ord(
            message.data[5]), ord(message.data[6]))
...
def callback_direction(message):
    ...
    if flag_manul == 0:
        print("Received from bluetooth: header=%d,direction=%d,speed=%d,gear==%d,manual=%d,beep=%d,crc=%d" %
              (ord(auto_driver_data[0]), ord(auto_driver_data[1]), ord(auto_driver_data[2]), ord(auto_driver_data[3]), ord(auto_driver_data[4]), ord(auto_driver_data[5]), ord(auto_driver_data[6])))
...
def callback_speed(message):
    ...
    auto_driver_data = auto_driver_data[0:6] + crc + auto_driver_data[7:]
    if flag_manul == 0:
        print("Receiived from bluetooth: header=%d,direction=%d,speed=%d,gear==%d,manual=%d,beep=%d,crc=%d" %
              (ord(auto_driver_data[0]), ord(auto_driver_data[1]), ord(auto_driver_data[2]), ord(auto_driver_data[3]), ord(auto_driver_data[4]), ord(auto_driver_data[5]), ord(auto_driver_data[6])))
        ser.write(auto_driver_data[0:8])
        ser.flush()
...
#            ser.baudrate=9600
            ...
            while not rospy.is_shutdown():
                ...
                if True:
                    ...
                    print("batteryVoltage:", unpack_data[31])
                    vcu_motorTemperature_pub.publish(unpack_data[32])
                    print(unpack_data[0], unpack_data3[0], unpack_data1[0], unpack_data1[1], unpack_data1[2], unpack_data1[3], unpack_data1[4], unpack_data1[5], unpack_data1[6], unpack_data1[7], unpack_data1[8], unpack_data1[9], unpack_data1[10],
                          unpack_data1[11], unpack_data1[12], unpack_data1[13], unpack_data1[14], unpack_data[31], unpack_data[32])
                #rate.sleep()
